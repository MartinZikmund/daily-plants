...
public class ExportRoundTripTests
{
    ...
    [TestMethod]
    public async Task Import_WithAnUnreadableAchievementTimestamp_SkipsTheRow()
    {
        var json = """
        {
          "version": "1.1",
          "dailyEntries": [],
          "weightEntries": [],
          "achievements": [ { "achievementId": "streak_7", "earnedAt": "not-a-timestamp", "hasBeenSeen": true } ]
        }
        """;

        var result = await _service.ImportFromJsonAsync(json);

        result.AchievementsImported.Should().Be(0);
        result.EntriesSkipped.Should().Be(1,
            "coercing the timestamp to now would silently rewrite when the badge was earned");
        (await _data.GetEarnedAchievementsAsync()).Should().BeEmpty();
    }
